...
function select_blast () {
    ...
    _rmbl_exec_i=$(find . -name "ncbi-rmblastn-*.tar.gz" -print)
...
function install_repeatmasker () {
    ...
    _rm=RepeatMasker-open-4-0-3.tar.gz
    ...
    if ! command -v /usr/local/rmblast/bin/blastn >/dev/null 2>&1; then
        select_blast
        cd /usr/local/
        _rmbldir=$(find . -type d -name ncbi-rmblastn-* -print)
